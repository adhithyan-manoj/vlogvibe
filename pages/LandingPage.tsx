  ...
  useEffect(() => {
    if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
      ...
      tl.from('.hero-title', { y: 100, opacity: 0, duration: 1.2, ease: 'power4.out' })
        ...
        .from('.hero-cta', { y: 20, opacity: 0, duration: 0.6, ease: 'power2.out', stagger: 0.2 }, '-=0.4');
  ...
  return (
    <div className="relative pt-20 md:pt-24 pb-20 px-4 md:px-6 overflow-hidden">
      ...
      <section className="min-h-[70vh] md:min-h-[85vh] flex flex-col items-center justify-center text-center max-w-6xl mx-auto">
        <div className="hero-sub inline-block px-3 py-1 mb-6 md:mb-8 glass rounded-full text-[10px] md:text-xs font-bold tracking-[0.2em] text-purple-400 uppercase border border-purple-500/20">
          The New Era of Vlogging
        </div>
        <h1 className="hero-title text-4xl sm:text-6xl md:text-9xl font-extrabold mb-6 md:mb-8 tracking-tighter leading-[1.1] md:leading-[0.95] max-w-5xl">
          Cinema in <br />
          <span className="gradient-text">Every Story.</span>
        </h1>
        <p className="hero-sub text-base md:text-2xl text-gray-400 mb-8 md:mb-12 max-w-2xl font-light leading-relaxed px-4">
          A boutique platform for world-class creators and fans who demand the highest production quality.
        </p>
        <div className="hero-cta flex flex-col sm:flex-row gap-4 md:gap-6 w-full sm:w-auto">
      ...
      <section ref={addToRefs} className="max-w-4xl mx-auto py-24 md:py-48 text-center">
        <div className="px-2">
      ...
      <section ref={addToRefs} className="max-w-7xl mx-auto py-16 md:py-32 border-t border-white/5">
      ...
      <section ref={addToRefs} className="max-w-5xl mx-auto py-16 md:py-32 text-center px-2">
